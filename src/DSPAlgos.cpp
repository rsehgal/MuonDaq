...
double DSPAlgos::CalculateInterpolatedValue(double x1, double y1, double x2, double y2) {
  return x1 - (y1 * ((x2 - x1) / (y2 - y1)));
}


void DSPAlgos::CalculateFineTStamp(std::vector<short> signal, bool ns) {
fFineTStamp = fCoarseTStamp * 1000 - CalculateZeroCrossing(signal,ns) * 1000;
}

double DSPAlgos::CalculateZeroCrossing(std::vector<short> signal, bool ns) {
  ...
  //std::cout << RED << "idx1 : " << idx1 << " : idx2 : " << idx2 << RESET << std::endl;
  double x1 = idx1*4.;
  double y1 = signal[idx1];
  double x2 = idx2*4.;
  double y2 = signal[idx2];
  std::cout << std::setprecision(15);
  //std::cout << GREEN << "ZeroCrossingValue  : " <<  CalculateInterpolatedValue(x1,y1,x2,y2) << RESET << std::endl;
  zero_crossing = CalculateInterpolatedValue(x1,y1,x2,y2);
  return (zero_crossing-x1);
  ...
  // std::cout << "TriggerStart : " << trigger_start << " : ZeroCrossing : " << zero_crossing << " : FineTStamp : " <<
  // fine_timestamp << std::endl;
