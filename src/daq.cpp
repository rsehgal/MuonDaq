...
int startUPDServer(int thread_num, int portnum) {
  ...
  */
  ...
  while (!stop_flag.load()) {
    if(eventCounter > 1){
    if((treeVec[boardId]->currentTStamp-prevTStamp) > 300){
     std::cout <<"Eventcounter : " << eventCounter << " : CurrentTStamp : " << treeVec[boardId]->currentTStamp <<" : prevTStamp : " << prevTStamp << std::endl;
    ...
    */
    ...
    treeVec[boardId]->fineTStampNear = (Coarse_time_stamp_Near-(1.*treeVec[boardId]->fineTStampNear/65536.))*4;
